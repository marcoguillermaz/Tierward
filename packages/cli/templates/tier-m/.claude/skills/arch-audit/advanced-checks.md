...
**Probe-before-trust (WARN, review-time - not frontmatter-visible):** the FAIL above catches a skill whose *frontmatter* is set to `haiku`. It cannot catch the subtler regression - an edit to the **expected-state table itself** that blesses a lower tier for a judgment skill on cost grounds. Per the model-tiering house rule in `docs/model-effort-policy.md`, a cost-driven de-escalation of a judgment step is trusted only after a head-to-head probe against a known-answer defect. So when that file is present, any expected-state entry set **below** its rubric-derived tier must cite a **dated probe note** in the file's Probe log; WARN on any entry lowered without one. There is no grep for this - it is a review-time read of the table against the Probe log. If `docs/model-effort-policy.md` is absent (e.g. Tier S), skip this check: there is no rubric baseline or probe log to reconcile against.
